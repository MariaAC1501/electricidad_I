...
\begin{itemize}
    ...
    \item Celda electroquímica recargable: es un dispositivo capaz de convertir energía química en energía eléctrica y viceversa. 
...
Existen gran cantidad de métodos para determinar los cinco parámetros y con ellos darle valores a los elementos del circuito mostrado en la Figura \ref{fig:OneDiode}; sin embargo, todos ellos están fuera del alcance de este curso. El método desarrollado por Villava \cite{villalva2009modeling} permite construir el modelo usando la información que encuentra en la hoja de datos de la celda, esa información incluye los siguientes valores: 
\begin{itemize}
    \item $V_{oc}$: \emph{open circuit voltage}, voltaje de circuito abierto. Es el voltaje que tiene el panel cuando no tiene carga conectada en sus terminales, o sea, cuando la corriente es igual a cero.
    \item $I_{sc}$ \emph{short circuit current}, corriente de corto circuito. En la corriente que se obtendría si la terminales se ponen en cortocircuito, o sea, cuando el voltaje es cero. 
    \item $V_{mp}$ \emph{maximum power voltage}, voltaje en el punto de máxima potencia, o sea, el voltaje al que se alcanza el punto máximo en la curva P-V de la Figura \ref{fig:real_plots}
    \item $I_{mp}$ \emph{maximum power current}, corriente en el punto de máxima potencia, o sea, la corriente correspondiente a el punto máximo en la curva P-V de la Figura \ref{fig:real_plots}
\end{itemize}

Si usamos el método de Villalva con un arreglo de 2 celdas en serie y 3 en paralelo (2s3p), usando la celda modelo 3G30A de Azurspace (ver Figura \ref{fig:3G30A}), obtendríamos una curva similar a la de la Figura \ref{fig:real_plots}. Esto es a una temperatura de $\SI{28}{\celsius}$, una irradiación igual a la constante solar, o sea $\SI{1367}{\watt\meter^{-2}}$ y al inicio de su vida útil (BOL, beginning of life). Los datos importantes para poder construir el modelo de un arreglo 2s3p están en la Tabla \ref{t:cell}
