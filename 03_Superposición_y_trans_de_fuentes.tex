...
\emph{El teorema de superposición establece que en cualquier circuito lineal con más de una fuente independiente, la respuesta a través de cualquier elemento es la suma de las respuestas obtenidas debido a cada fuente independiente considerada por separado y con todas las demás fuentes independientes apagadas.}
    ...
    %     (2,3)
    %     ;
    %     \draw[white] (-1,-1) rectangle (5,4);
    %     \draw (2,-1) node[above]{$v_x$};
    %     \end{circuitikz}
    % \end{column}
    ...
    %         to[R, l=$R_1$]
    %     (2,3) -- (4,3)
    %         to[R,l=$R_2$,v=$v_{xv_1}$]
    %     (4,0) -- (0,0)
    %     ;
    %     \draw[white] (-1,-1) rectangle (5,4);
    %     \draw (2,-1) node[above]{$v_{xv_1} = v_1 \cdot \frac{R_2}{R_1+R_2}$};
    %     \end{circuitikz}
    % \end{column}
    % \begin{column}{0.05\textwidth}
    % {\huge$+$}
    ...
    %     ;
    %     \draw[white] (-1,-1) rectangle (5,4);
    %     \draw (2,-1) node[above]{$v_{xi_1} = i_1 \cdot \frac{R_1\cdot R_2}{R_1+R_2}$};
    %     \end{circuitikz}
    % \end{column}
    % \end{columns}
\end{frame}
